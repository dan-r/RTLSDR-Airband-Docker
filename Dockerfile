...
RUN apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 3B4FE6ACC0B21F32 && apt-get update && apt-get install -y libshout3=2.4.1-2build1 libshout3-dev=2.4.1-2build1
...
RUN cd / && rm -rf /build && apt-get -y purge build-essential cmake pkg-config wget gnupg && rm -rf /var/lib/apt/lists/*
